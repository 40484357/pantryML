...
#I found that there were 9960 unique users and 10k unique recipes. 
#print('unique users:',len(_part.user_id.unique()))
#print('unique recipes:',len(_part.recipe_id.unique()))

#I wanted to see data on the amount of reviews a user has given from this data frame as well as the number of reviews a recipe receives. 
...
def getRecommendations_UserBased(user_id, top_n = 10):
    ...
    movie_rated = list(clean_df['recipe_id'].loc[clean_df['user_id'] == user_id])
    ...
    for recipe_id in list(dict_top_n[0].keys()):
        for old_recipe, new_recipe in set_recipeID.items():
            if recipe_id == new_recipe:
                ...
                print(f'Top {i} Original Recipe ID: {new_recipe} - {name}\n Ingredients: {ingredients}\n')
                
                i += 1
                
    return dict_top_n[0]

R1_user = getRecommendations_UserBased(811)
print(R1_user)
